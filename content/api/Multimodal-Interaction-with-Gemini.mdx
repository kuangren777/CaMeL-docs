...
<CodeGroup>
  ...
  client = genai.Client(
      ...
      http_options={"base_url": "@@CONSOLE_BASE@@/gemini"},
  )
...
VEO 3.0 是由 Google DeepMind 开发的最新先进视频生成模型。使用 [VEO 3.0](@@CONSOLE_BASE@@/models?model=veo-3.0-generate-preview)，您可以生成具有以下特点的视频：
...
<CodeGroup>
  ...
  curl "@@CONSOLE_BASE@@/gemini/v1beta/models/veo-3.0-generate-preview:predictLongRunning?key=sk-***" \
    -H "Content-Type: application/json" \
    -X "POST" \
    -d '{
      "instances":
  ...
  curl "@@CONSOLE_BASE@@/gemini/v1beta/models/veo-3.0-generate-preview/operations/ff5***?key=sk-***"
  ```
