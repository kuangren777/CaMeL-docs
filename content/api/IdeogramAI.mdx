...
`POST` [@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/generate](@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/generate)
...
<CodeGroup>
  ...
  curl -X POST @@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/generate \
    -H "Api-Key: sk-***" \
  ...
  response = requests.post(
    "@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/generate",
    headers={
...
`POST` [@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/remix](@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/remix)
...
<CodeGroup>
  ...
  with open(source_image_path, "rb") as image_file:
      ...
      response = requests.post(
        "@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/remix",
        headers={
          "Api-Key": "sk-***" # 换成你在 CaMeL AI 生成的密钥
        },
        data=data,
...
`POST` [@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/edit](@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/edit)
...
<CodeGroup>
  ...
  with open(source_image_path, "rb") as image_file, open(mask_image_path, "rb") as mask_file:
      response = requests.post(
          "@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/edit",
          headers={
              "Api-Key": "sk-***" # 换成你在 CaMeL AI 生成的密钥
          },
          data={
...
`POST` [@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/replace-background](@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/replace-background)
...
<CodeGroup>
  ...
  with open(source_image_path, "rb") as image_file:
      ...
      response = requests.post(
        "@@CONSOLE_BASE@@/ideogram/v1/ideogram-v3/replace-background",
        headers={
          "Api-Key": "sk-***" # 换成你在 CaMeL AI 生成的密钥
        },
        data=data,
...
`POST` [@@CONSOLE_BASE@@/ideogram/generate\\](@@API_BASE@@/ideogram/generate\\) 根据给定提示词和可选参数同步生成图像。图像链接的有效期有限；如果你想保留图像，必须下载保存。
...
<CodeGroup>
  ...
  url = "@@CONSOLE_BASE@@/ideogram/generate"
  ...
  const url = '@@CONSOLE_BASE@@/ideogram/describe';
  ...
  curl -X POST @@CONSOLE_BASE@@/ideogram/generate \
       -H "Api-Key: <apiKey>" \
...
`POST` [@@CONSOLE_BASE@@/ideogram/edit](@@API_BASE@@/ideogram/edit)
...
<CodeGroup>
  ...
  url = "@@CONSOLE_BASE@@/ideogram/eidt"
  ...
  ```js Javascript theme={null}
  const url = '@@CONSOLE_BASE@@/ideogram/edit';
  const form = new FormData();
  form.append('image_file', '<file1>');
  ...
  ```shell Curl theme={null}
  curl -X POST @@CONSOLE_BASE@@/ideogram/edit \
       -H "Api-Key: <apiKey>" \
       -H "Content-Type: multipart/form-data" \
       -F image_file=@<file1> \
       -F mask=@<file1> \
...
`POST` [@@CONSOLE_BASE@@/ideogram/remix](@@API_BASE@@/ideogram/remix)
...
<CodeGroup>
  ...
  url = "@@CONSOLE_BASE@@/ideogram/remix"
  ...
  ```js Javascript theme={null}
  const url = '@@CONSOLE_BASE@@/ideogram/remix';
  const form = new FormData();
  ...
  ```shell Curl theme={null}
  curl -X POST @@CONSOLE_BASE@@/ideogram/remix \
       -H "Api-Key: <apiKey>" \
       -H "Content-Type: multipart/form-data" \
       -F image_request='{
    "prompt": "A serene tropical beach scene. Dominating the foreground are tall palm trees with lush green leaves, standing tall against a backdrop of a sandy beach. The beach leads to the azure waters of the sea, which gently kisses the shoreline. In the distance, there is an island or landmass with a silhouette of what appears to be a lighthouse or tower. The sky above is painted with fluffy white clouds, some of which are tinged with hues of pink and orange, suggesting either a sunrise or sunset.",
...
`POST` [@@CONSOLE_BASE@@/ideogram/upscale](@@API_BASE@@/ideogram/upscale)
...
<CodeGroup>
  ...
  url = "@@CONSOLE_BASE@@/ideogram/upscale"
  ...
  ```js Javascript theme={null}
  const url = '@@CONSOLE_BASE@@/ideogram/upscale';
  const form = new FormData();
  form.append('image_request', '{}');
  form.append('image_file', '<file1>');
  ...
  ```shell Curl theme={null}
  curl -X POST @@CONSOLE_BASE@@/ideogram/upscale \
       -H "Api-Key: <apiKey>" \
       -H "Content-Type: multipart/form-data" \
       -F image_request='{}' \
       -F image_file=@<file1>
...
`POST` [@@CONSOLE_BASE@@/ideogram/describe](@@API_BASE@@/ideogram/describe)
...
<CodeGroup>
  ...
  url = "@@CONSOLE_BASE@@/ideogram/describe"
  ...
  ```shell Curl theme={null}
  curl -X POST @@CONSOLE_BASE@@/ideogram/describe \
       -H "Api-Key: <apiKey>" \
       -H "Content-Type: multipart/form-data" \
       -F image_file=@<file1>
  ```
