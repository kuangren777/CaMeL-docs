...
**接口地址**：`GET @@CONSOLE_BASE@@/api/v1/models`
...
<CodeGroup>
  ...
  # 接口地址
  url = "@@CONSOLE_BASE@@/api/v1/models"
  ...
  fetch('@@CONSOLE_BASE@@/api/v1/models')
  ...
  fetch(`@@CONSOLE_BASE@@/api/v1/models?${params}`)
    .then(response => response.json())
    .then(data => console.log(data));
  ```

  ```bash cURL theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/v1/models"

  curl -X GET "@@CONSOLE_BASE@@/api/v1/models?type=llm&modalities=text&model=gpt-5&features=thinking&sort_by=context_length&sort_order=desc"
  ```
</CodeGroup>
...
<CodeGroup>
  ...
  GET @@CONSOLE_BASE@@/api/v1/models?type=llm
  ```

  ```bash 获取适合编程的模型，按上下文长度排序 theme={null}
  GET @@CONSOLE_BASE@@/api/v1/models?tag=coding&sort_by=context_length&sort_order=desc
  ```

  ```bash 搜索特定模型 theme={null}
  GET @@CONSOLE_BASE@@/api/v1/models?model=gpt-5
  ```

  ```bash 复合条件查询 theme={null}
  GET @@CONSOLE_BASE@@/api/v1/models?type=llm&modalities=text,image&features=function_calling&sort_by=model_ratio&sort_order=asc
  ```

  ```bash 编程模型智能排序 theme={null}
  GET @@CONSOLE_BASE@@/api/v1/models?sort_by=coding
  ```
</CodeGroup>
