...
1. 一个 [CaMeL AI 账户](@@CONSOLE_BASE@@)
2. 从 [CaMeL AI 设置页面](@@CONSOLE_BASE@@/setting) 点击「生成系统访问令牌」生成访问令牌 (Access Token)；
3. 安装必要的 Python 依赖项：
...
<CodeGroup>
  ...
  curl -X GET "@@CONSOLE_BASE@@/api/user/self" \
  ...
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_balance
  ```

  ```shell CLI 获取 Key 的可用额度 theme={null}
  curl '@@CONSOLE_BASE@@/dashboard/billing/remain' \
...
<CodeGroup>
  ...
  curl -X POST "@@CONSOLE_BASE@@/api/token/" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{
      "name": "新Key名称",
  ...
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action create_token --name "新Key名称" --expires -1 --quota 500000
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/token/?num=20" \ #修改num参数可以调整输出的数据数量。
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_tokens
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/token/search?keyword=搜索关键词" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action search_tokens --query "搜索关键词"
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X PUT "@@CONSOLE_BASE@@/api/token/" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{
      "id": "Key_ID",
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action update_token --id "Key_ID" --name "新名称" --expires 86400 --quota 100000 --status 1
  ```
</CodeGroup>
...
<CodeGroup>
  ...
  curl -X DELETE "@@CONSOLE_BASE@@/api/token/Key_ID" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action delete_token --id "Key_ID"
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/user/token" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_user_token
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/models" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_models
  ```
</CodeGroup>
...
<CodeGroup>
  ```shell Curl theme={null}
  curl -X GET "@@CONSOLE_BASE@@/api/user/available_models" \
    -H "Authorization: YOUR_ACCESS_TOKEN" \
    -H "Content-Type: application/json"
  ```
  ...
  ```shell CLI theme={null}
  python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_available_models
  ```
</CodeGroup>
...
```bash  theme={null}
python camelai_cli.py --url "@@CONSOLE_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_balance --json
```
...
1. **连接问题**：如果主域名连接失败，尝试使用备用域名：
   ...
   ```bash  theme={null}
   python camelai_cli.py --url "@@API_BASE@@" --token "YOUR_ACCESS_TOKEN" --action get_balance
   ```
