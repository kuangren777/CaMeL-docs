...
<Tabs>
  <Tab title="TypeScript">
    ...
    const config = {
      ...
      baseURL: '@@CONSOLE_BASE@@',   // 替换为 CaMeL AI 网关
      headers: {
        'APP-Code': 'APP Code'  // 应用Code可从@@CONSOLE_BASE@@/appstore获取
      }
    };
    ...
    function routeModel(modelName: string) {
      if (modelName.startsWith('claude')) {
        ...
        // Gemini 模型：使用 Google SDK，端点 @@CONSOLE_BASE@@/gemini
        return 'gemini';
      } else {
        // 其他模型：使用 OpenAI 兼容接口
        return 'openai';
  ...
  <Tab title="Python">
    ...
    credentials = {
        ...
        "base_url": "@@CONSOLE_BASE@@",   # 替换为 CaMeL AI 网关
        "extra_headers": {
            "APP-Code": "APP Code"  # 应用Code可从@@CONSOLE_BASE@@/appstore获取
        }
    }
...
<Tabs>
  <Tab title="TypeScript">
    ...
    const config = {
      ...
      baseURL: process.env.CAMELAI_BASE_URL || '@@CONSOLE_BASE@@',
  ...
  <Tab title="Python">
    ...
    config = {
        ...
        "base_url": os.getenv("CAMELAI_BASE_URL", "@@CONSOLE_BASE@@"),
...
* **多客户端路由**：
  ...
  * `gemini*`/`imagen*` 且不以 `-nothink`/`-search` 结尾且不包含 `embedding` → 使用 Gemini 客户端（`apiHost: @@CONSOLE_BASE@@/gemini`）
...
* Gemini 客户端使用 `@@CONSOLE_BASE@@/gemini` 作为 `apiHost`
