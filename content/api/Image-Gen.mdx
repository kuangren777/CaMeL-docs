...
* [gpt-image-1.5 ](@@CONSOLE_BASE@@/model/gpt-image-1.5)—— OpenAI 最新的图像生成模型，它具有更好的指令遵循性和对提示的执行能力。
* [gpt-image-1](@@CONSOLE_BASE@@/model/gpt-image-1) —— 多模态语言模型，它接受文本和图像输入，并生成图像输出。
* [gpt-image-1-mini](@@CONSOLE_BASE@@/model/gpt-image-1-mini) —— gpt-image-1 的经济型版本。
* [dall-e-3](@@CONSOLE_BASE@@/model/dall-e-3) —— 能够根据自然语言描述创建逼真的图像和艺术作品,支持根据提示创建指定尺寸的新图像。

### Imagen 模型

* [imagen-4.0-ultra-generate-001](@@CONSOLE_BASE@@/model/imagen-4.0-ultra-generate-001)
* [imagen-4.0-generate-001](@@CONSOLE_BASE@@/model/imagen-4.0-generate-001)
* [imagen-4.0-fast-generate-001](@@CONSOLE_BASE@@/model/imagen-4.0-fast-generate-001)
* [imagen-4.0-fast-generate-preview-06-06](@@CONSOLE_BASE@@/model/imagen-4.0-fast-generate-preview-06-06)
* [imagen-3.0-generate-002](@@CONSOLE_BASE@@/model/imagen-3.0-generate-002)

### Qwen 模型

* [qwen-image](@@CONSOLE_BASE@@/model/qwen-image) —— Qwen 系列中的一个图像生成基础模型，在复杂文本渲染和精确图像编辑方面取得了显著进展。
* [qwen-image-edit](@@CONSOLE_BASE@@/model/qwen-image-edit) —— Qwen 图像编辑模型。

### Doubao 模型

* [doubao-seedream-5.0-lite](@@CONSOLE_BASE@@/model/doubao-seedream-5.0-lite) —— 字节跳动发布的最新图像创作模型，该模型首次搭载联网检索功能，能融合实时网络信息，提升生图时效性。

* [doubao-seedream-4-5](@@CONSOLE_BASE@@/model/doubao-seedream-4-5) —— 字节跳动推出的图像多模态模型，整合了文生图、图生图、组图输出等能力。

* [doubao-seedream-4-0](@@CONSOLE_BASE@@/model/doubao-seedream-4-0) —— 字节跳动SOTA级多模态图像创作模型，支持多图融合创作、图像编辑、组图生成等多样玩法。

### Flux 模型

* [flux-2-flex ](@@CONSOLE_BASE@@/model/flux-2-flex)—— 专为真实世界的创意生产工作流打造，在生成高质量图像的同时，能够保持多参考图之间的角色与风格一致性。
* [flux-2-pro ](@@CONSOLE_BASE@@/model/flux-2-flex)—— 黑森林实验室研发的图片生成与编辑模型，支持复杂文本的读写。
* [FLUX.1-Kontext-pro](FLUX.1-Kontext-pro) ——  黑森林实验室研发的多模态流匹配模型，既支持文本到图像的生成，又支持在上下文中进行图像编辑。

### iRAG 模型

* [irag-1.0](@@CONSOLE_BASE@@/model/irag-1.0) —— 百度自研生图模型，能够生成超真实图片。
* [ernie-irag-edit](@@CONSOLE_BASE@@/model/ernie-irag-edit) —— 百度自研的图像编辑模型，支持基于图片进行 erase、repaint、variation 等操作。

### Ideogram 模型

* [V3](@@CONSOLE_BASE@@/model/V3) —— Ideogram AI 生图模型，能够快速生成图片。
...
POST @@API_BASE@@/models/<model_path>/predictions
```
...
<CodeGroup>
  ...
  curl @@API_BASE@@/models/openai/gpt-image-1.5/predictions \
  ...
  curl @@API_BASE@@/models/openai/gpt-image-1/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
...
<CodeGroup>
  ...
  curl @@API_BASE@@/models/google/imagen-4.0-fast-generate-001/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
...
<CodeGroup>
  ...
  curl @@API_BASE@@/models/qianfan/qwen-image/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
    -d '{
      "input": {
  ...
  curl @@API_BASE@@/models/qianfan/qwen-image-edit/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
...
<CodeGroup>
  ```shellscript 文生图 theme={null}
  curl @@API_BASE@@/models/doubao/doubao-seedream-4-0/predictions \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer sk-***" \
    -d '{
      "input": {
  ...
  ```shellscript 图生图 theme={null}
  curl @@API_BASE@@/models/doubao/doubao-seedream-4-5/predictions \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer sk-***" \
    -d '{
      "input": {
...
```shellscript  theme={null}
curl @@API_BASE@@/images/generations \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer sk-***" \
    -d '{
...
<CodeGroup>
  ...
  curl @@API_BASE@@/models/bfl/flux-kontext-max/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
...
<CodeGroup>
  ```shell Curl theme={null}
  curl @@API_BASE@@/models/ideogram/V3/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
...
<CodeGroup>
  ...
  curl @@API_BASE@@/models/qianfan/irag-1.0/predictions\
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
    -d '{
      "input": {
  ...
  curl @@API_BASE@@/models/qianfan/ernie-irag-edit/predictions \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-***" \
      -d '{
    "input": {
